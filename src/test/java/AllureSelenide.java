    ...
    public void afterEvent(LogEvent logEvent) {
        lifecycle.getCurrentTestCase().ifPresent(uuid -> {
            ...
            if (LogEvent.EventStatus.FAIL.equals(logEvent.getStatus())) {
                ...
                lifecycle.addAttachment("Html source", "text/html", "html", getPageSourceBytes());
                lifecycle.updateStep(stepResult -> {
                    final StatusDetails details = ResultsUtils.getStatusDetails(logEvent.getError())
                            .orElse(new StatusDetails());
                    stepResult.setStatusDetails(details);
            ...
            lifecycle.updateStep(stepResult -> stepResult.setStart(stepResult.getStart() - logEvent.getDuration()));
            lifecycle.stopStep(stepUUID);
        });
    }
